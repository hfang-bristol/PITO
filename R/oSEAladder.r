...
#'
#' \code{oSEAladder} is supposed to visualise enrichment results using ladder-like plot in which rows for terms and columns for its members. The members are sorted first by sharings and then by individual terms. It returns an object of class "ggplot". 
#'
#' @param eSET an object of class "eSET" or a tibble (or a data frame)
#' @param sortBy which statistics will be used for sorting and viewing gene sets (only works when applied to the eSET object). It can be "adjp" or "fdr" for adjusted p value (FDR), "pvalue" for p value, "zscore" for enrichment z-score, "fc" for enrichment fold change, "nA" for the number of sets (terms), "nO" for the number in overlaps, "or" for the odds ratio, and "none" for ordering according to ID of terms
...
#' gp_ladder <- oSEAladder(eTerm, sortBy="none", top_num="auto", FDR.cutoff=1)
#' gp_ladder+ coord_flip()
#' 
#' # 3) save into the file "oSEAladder.pdf"
#' mat <- xSparseMatrix(gp_ladder$data)
#' pdf("oSEAladder.pdf", width=2+ncol(mat)*0.075, height=2+nrow(mat)*0.1, compress=T)
#' print(gp_ladder)
#' dev.off()
#' 
#' # 4) SIFTS2GOMF
...
#' gp_ladder <- oSEAladder(eTerm, sortBy="none", top_num=5, FDR.cutoff=0.01, x.rotate=90)
#' #gp_ladder + coord_flip()
#' ## data_matrix
...
#' ## gp_pdb
#' gp_pdb <- oHeatmap(t(data_matrix), reorder="row", colormap="jet.top", x.rotate=90, shape=19, size=1, x.text.size=6,y.text.size=5, na.color='transparent', legend.title='# pockets')
#' #gp_pdb + coord_flip()
#' ## plot_combined
...
oSEAladder <- function(eSET, sortBy=c("fdr","or","adjp","pvalue","zscore","fc","nA","nO","none"), top_num=5, FDR.cutoff=0.05, CI.one=T, colormap="lightgrey-grey-black", x.rotate=90, x.text.size=6, y.text.size=6, shape=22, size=2, label=c('full','concise'), verbose=T, ...)
{
    ...
    if(is(eSET,'eSET')){
		...
		df_enrichment <- eSET %>% oSEAextract(sortBy=sortBy) %>% dplyr::slice(1:top_num)
	
	}else if(is(eSET,'data.frame')){
		df_enrichment <- eSET
	}
		...
		if(!is.null(df_enrichment)){
			...
			if(label=='concise'){
				#df_enrichment$label <- paste0(df_enrichment$name, " [OR=", df_enrichment$or, ", FDR=", df_enrichment$adjp, ", n=", df_enrichment$nO, "]")
				df_enrichment$label <- paste0(df_enrichment$name, " [n=", df_enrichment$nO, "/", df_enrichment$nA, "]")
			}else{
				df_enrichment$label <- paste0(df_enrichment$name, "\n[OR=", df_enrichment$or, ", P=", df_enrichment$pvalue, ", FDR=", df_enrichment$adjp, ", n=", df_enrichment$nO, "/", df_enrichment$nA, "]")
			}
			...
			if(length(ls_path)>=1){
				...
				if(1){
					...
					gp_heatmap <- oHeatmap(mat_heatmap, reorder="none", colormap=colormap, zlim=c(0,max(mat_heatmap,na.rm=T)), ncolors=64, barwidth=0.4, x.rotate=x.rotate, x.text.size=x.text.size, y.text.size=y.text.size, shape=shape, size=size, na.color='transparent', ...)
