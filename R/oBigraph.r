...
#'
#' \code{oBigraph} is supposed to obtain communities from a bipartitle graph.
#'
...
oBigraph <- function(g, algorithm=c("louvain","leading_eigen","fast_greedy","walktrap"), seed=825, glayout=layout_with_kk, edge.color.alternative=c("grey70","grey95"), verbose=TRUE)
{
	...
	ig <- oNetInduce(ig, nodes_query=V(ig)$name, knn=0, largest.comp=F)
	...
	################
	if(length(unique(df_edges$from))==1 | length(unique(df_edges$to))==1){
		ig_converted <- ig
		V(ig_converted)$community <- 1
		return(ig_converted)
	}
	################
	...
	sM <- Matrix::sparseMatrix(i=ynodes, j=xnodes, x=weights, dims=c(length(unique(ynodes)),length(unique(xnodes)))) %>% as.matrix()
	...
	ig_x <- oNetInduce(ig_x, nodes_query=V(ig_x)$name, knn=0, largest.comp=F)
	...
	#############################
	ig_converted <- oAddCoords(ig_converted, node.attr="community", seed=seed, glayout=glayout, edge.color.alternative=edge.color.alternative, verbose=verbose)
